...
# ----------------- Admin Email Notification (safe SMTP + console fallback) -----------------
# Defense explanation script:
# "We implemented a Django SMTP-based email notification system. For reliability during demonstration,
#  we added a fallback console backend mode. This ensures notifications always work even if external SMTP
#  services are unavailable, making the system demo-safe and robust." 

from django.views.decorators.http import require_GET
from .email_config import get_email_from, get_notification_recipient_list
...
def send_notification_email(request):
    try:
        ...
        send_mail(
            subject="AlertHub Notification",
            message="This is a system notification email from AlertHub.",
            from_email=get_email_from(),
            recipient_list=get_notification_recipient_list(),
            fail_silently=False,
        )

        return JsonResponse({
            "status": "success",
            "message": f"Email processed successfully (mode={mode})",
        })

    except Exception as e:
        return JsonResponse({
            "status": "fallback",
            "message": "Email failed (check console mode if enabled)",
            "error": str(e),
        })
