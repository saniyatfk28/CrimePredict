...
export default function LawUserChat({
  citizenId,
  lawyerId,
  incidentId,
  title,
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      // no-op (server pushes cached history)
    };
      ...
      <div className="card-body p-0">
        <div className="p-3" style={{ maxHeight: '340px', overflowY: 'auto' }}>
          {messages.length === 0 ? (
            ...
            messages.map((m, idx) => {
              const isLaw = m.role === 'law';
              const align = isLaw ? 'text-end' : 'text-start';
              const bubbleClass = isLaw
